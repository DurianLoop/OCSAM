...
class Matcher:
    ...
    def predict(self):
        ...
        center_boxes = self.create_center_boxes(centers)
    ...
    def save_points_to_csv(self, points):
        # 创建保存目录
        save_dir = f'MonuSeg/Match_Points/'
    ...
    def visualize_merged_mask(self, target_image, merged_mask, save_path=None):
        ...
        # 转换 partial mask (reference mask) 格式
        ...
        # 1. Original Image with Partial Mask (Support/Reference)
        ...
        # 3. Original Image with Predicted Mask
        axes[2].imshow(img_display)
        axes[2].imshow(mask_display, alpha=0.5, cmap='Reds')
    ...
    def mask_generation(self, tar_img_np, center_boxes):
        ...
        if hasattr(self, 'tar_img') and self.tar_img is not None:
            save_path = f'MonuSeg/boxes/centers_boxes_{self.image_counter:04d}.png'
        ...
        # 可视化 SAM 输出
        sam_save_path = f'MonuSeg/sam/sam_outputs_{self.image_counter:04d}.png'
        ...
        merged_save_path = f'MonuSeg/merged/merged_mask_{self.image_counter:04d}.png'
...
def build_matcher_oss(args):
    ...
    # # 默认参数（适用于 MonuSeg 数据集）
    num_centers = getattr(args, 'num_centers', -1)
    # num_centers = all_points
    box_size = getattr(args, 'box_size', 20)
